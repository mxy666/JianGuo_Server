...
public class T_user_moneyout_List_OK_Servlet extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		List<T_user_moneyout_Bean> list_t_user_moneyout = T_user_moneyout_Sql.select_All_okss("0");
...
//			list_t_user_moneyout2.add(t_user_moneyout);
			...
			if(city_id.equals(t_job.getCity_id()+"")){
				list_t_user_moneyout2.add(t_user_moneyout);
				T_user_moneyout_Bean t_u_m = list_t_user_moneyout.get(i);
				d += t_u_m.getMoney();
			}
		}
		...
		request.setAttribute("sum", list_t_user_moneyout2.size());
